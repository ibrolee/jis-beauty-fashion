...
export function PurchasePanel({ product }: { product: ProductDetail }) {
  ...
  // The mobile option picker is above the image. Never carry more items into
  // the cart than the newly chosen variant has available.
    ...
    <div className="space-y-4 lg:space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="flex h-12 w-full items-center border border-line sm:w-36">
          <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} disabled={soldOut || safeQuantity <= 1} className="flex h-full w-12 items-center justify-center disabled:opacity-40" aria-label="Decrease quantity">
          ...
          <input
            ...
            aria-label="Quantity"
          />
          <button type="button" onClick={() => setQuantity((q) => Math.min(stock, q + 1))} disabled={soldOut || safeQuantity >= stock} className="flex h-full w-12 items-center justify-center disabled:opacity-40" aria-label="Increase quantity">
            <Plus className="h-4 w-4" />
          </button>
        </div>
        <AddToCartButton product={product} variant={variant} quantity={safeQuantity} className="flex-1" />
      ...
      {soldOut ? (
        ...
        <AddToCartButton product={product} variant={variant} quantity={safeQuantity} buyNow appearance="secondary" className="w-full" />
      )}

      <ul className="space-y-2 border-t border-line pt-4 text-sm text-ink-soft">
