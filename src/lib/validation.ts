...
export const addressSchema = z.object({
  ...
  addressLine: z.string().trim().min(5, "Enter your full delivery address"),
...
export const checkoutSchema = z.object({
  ...
  phone,
  whatsapp: phone.optional().or(z.literal("")),
  state: z.enum(NIGERIAN_STATES, { message: "Select your delivery state" }),
  city: z.string().trim().min(2, "City is required"),
...
export const contactSchema = z.object({
  ...
  message: z.string().trim().min(10, "Please write a short message").max(3000),
});
...
export const productSchema = z.object({
  ...
  volume: optionalText,
  topNotes: optionalText,
  heartNotes: optionalText,
  baseNotes: optionalText,
  ...
  /** One variant per line: "50ml | 26500 | 24000 | 20"  (name | price | salePrice? | stock) */
  variants: z.string().trim().optional().or(z.literal("")),
});
...
export function formToObject(formData: FormData, booleans: string[] = []): Record<string, unknown> {
  ...
  for (const b of booleans) obj[b] = formData.get(b) === "on" || formData.get(b) === "true";
  return obj;
}
