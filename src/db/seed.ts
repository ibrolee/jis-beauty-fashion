...
export async function seedDatabase(options: { force?: boolean } = {}) {
  ...
  // Admin credentials are optional for catalogue bootstrapping.
  // If they are configured, create the admin account.
  ...
  // Customer credentials are also optional.
  // Products/categories should still seed even if these are not configured.
  ...
  for (const [index, p] of seedProducts.entries()) {
    ...
    if (!categoryId) {
      throw new Error(`Unknown category slug: ${p.category}`);
    }
    ...
    const createdAt = new Date(
      now - (seedProducts.length - index) * 2 * 24 * 60 * 60 * 1000,
    );
    ...
    if (customerIds.length) {
      ...
      for (let r = 0; r < reviewCount; r++) {
        const template =
          seedReviewPool[(index + r * 3) % seedReviewPool.length];
        ...
        rows.push({
          ...
          createdAt: new Date(
            createdAt.getTime() + (r + 1) * 3 * 24 * 60 * 60 * 1000,
          ),
        });
      }
